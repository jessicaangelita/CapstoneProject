...
export async function getStaticPaths() {
    // Mendapatkan ID dari local storage
    const accessToken = localStorage.getItem("accessToken");
    const decodedToken = decodeToken(accessToken); // Anda perlu membuat fungsi decodeToken sesuai kebutuhan
  
    // Membuat path berdasarkan ID dari access token
    const paths = [
  ...
  export async function getStaticProps({ params }) {
    ...
    const response = await axios.get(
      `http://localhost:8050/user/profile/${params.id}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
        },
